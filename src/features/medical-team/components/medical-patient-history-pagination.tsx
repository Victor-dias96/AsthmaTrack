...
type MedicalPatientHistoryPaginationProps = {
  patientId: string;
...
/** In-memory page controls. Links stay on the medical history route. */
export function MedicalPatientHistoryPagination({
  patientId,
  currentPeriod,
  currentPage,
  totalPages,
}: MedicalPatientHistoryPaginationProps) {
  if (totalPages <= 1) {
    return null;
  }
  ...
  return (
    <nav aria-label="Paginação do histórico" className="min-w-0">
      <div className="flex min-w-0 flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
        {hasPrevious ? (
          <Link
            href={getMedicalPatientHistoryHref(
              patientId,
              currentPeriod,
              currentPage - 1
            )}
